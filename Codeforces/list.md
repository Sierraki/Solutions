## 🟢 第一阶段：思维与构造 (稳 A 关键)

*这类题不考套路，考查对规律的敏锐观察。*

* [X] **[1352A] Sum of Round Numbers** (800)
  *知识点：基础数字拆分* [🔗 题目链接](https://codeforces.com/problemset/problem/1352/A)
* [X] **[1367B] Even Array** (800)
  *知识点：奇偶校验与交换逻辑* [🔗 题目链接](https://codeforces.com/problemset/problem/1367/B)
* [X] **[1512C] A-B Palindrome** (1200)
  *知识点：回文串对称性与剩余计数* [🔗 题目链接](https://codeforces.com/problemset/problem/1512/C)
* [X] **[1520D] Same Differences** (1200)
  *知识点：移项变式 (a[j]-a[i]=j-i 转化为 a[j]-j=a[i]-i)* [🔗 题目链接](https://codeforces.com/problemset/problem/1520/D)

---

## 🟡 第二阶段：贪心与数学微调 (稳 B 冲击 C)

*这类题通常需要你给出一个“大致解”，然后根据差值进行修正。*

* [X] **[1475A] Odd Divisor** (900)
  *知识点：数论、2 的幂次判定* [🔗 题目链接](https://codeforces.com/problemset/problem/1475/A)
* [X] **[1360C] Similar Pairs** (1000)
  *知识点：贪心匹配与奇偶对数* [🔗 题目链接](https://codeforces.com/problemset/problem/1360/C)
* [ ] **[1335C] Two Teams Composed of Students** (1100)
  *知识点：Map/计数与集合大小平衡* [🔗 题目链接](https://codeforces.com/problemset/problem/1335/C)
* [ ] **[1186D] Vus the Cossack and Numbers** (1500)
  *知识点：总量守恒与局部微调 (你刚问过的那道题)* [🔗 题目链接](https://codeforces.com/problemset/problem/1186/D)

---

## 🔵 第三阶段：经典算法的 CF 变形 (力扣选手加分项)

*利用你擅长的算法，但在 CF 中需要更灵活的建模。*

* [ ] **[115A] Party** (900)
  *知识点：树的深度/森林最长链* [🔗 题目链接](https://codeforces.com/problemset/problem/115/A)
* [ ] **[1363A] Odd Selection** (1200)
  *知识点：分类讨论、奇偶性组合* [🔗 题目链接](https://codeforces.com/problemset/problem/1363/A)
* [ ] **[189A] Cut Ribbon** (1300)
  *知识点：完全背包变体 / 线性 DP* [🔗 题目链接](https://codeforces.com/problemset/problem/189/A)
* [ ] **[455A] Boredom** (1500)
  *知识点：权值 DP (类似打家劫舍，但需要先预处理频次)* [🔗 题目链接](https://codeforces.com/problemset/problem/455/A)
